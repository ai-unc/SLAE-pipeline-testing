...
def call_pipeline(data_path:str, settings_path:str, debug_path:str) -> Dict:
  ...
  return pipeline(data, model, prompt, 3, verbose)
  ...
  if EVALUATE:
      ...
      for paper in source[:n_papers]:
        prediction = call_pipeline(data_path=f"./pipeline_evaluator/full_dataset/{paper}",
                                    ...
                                    debug_path="./pipelines/iterative_summary_pipeline/debug_outputs")
        
        predictions.append(prediction)
      
      #score
      papers = []
      for p in source[:n_papers]:
